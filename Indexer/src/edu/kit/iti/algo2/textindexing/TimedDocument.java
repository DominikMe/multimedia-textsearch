 ...
 * 
 */
public class TimedDocument implements Serializable {
    private static final long serialVersionUID = 1L;
    private Set<TimeSlot> timeSlots = new TreeSet<>(
	    new TimeSlot.StartTimeComparator());
	...
	final String uuid = root.getAttributeValue("uuid");
	if (uuid != null && !uuid.isEmpty())
	    td.setUuid(uuid);

	for (Element slotE : root.getChildren()) {
	    td.add(TimeSlot.fromXmlElement(slotE, td));
	}

	return td;
    }

    public void correctTimeSlotOwnerShip() {
	for (TimeSlot s : timeSlots) {
	    s.setTimedDocument(this);
	}
    }
